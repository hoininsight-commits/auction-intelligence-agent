...
- **최신 커밋**: `6fdc964` "관리 페이지 리스트 UX 버그 3건 수정: 지역 부분일치 검색, 부동산/차량 탭, 정렬 UI"
...
4. ~~cron/스케줄러 미연결~~ → 2026-07-11 연결 완료: `app/workers/celery_app.py`에 `beat_schedule` 추가(온비드 매시간 `collect_source_items_task`, 국토부 실거래가 전국 순회 매일 새벽 3시 KST `collect_real_transaction_all_regions_task`). 국토부 전국 순회는 새 `real_transaction_connector.get_supported_regions()`으로 `_LAWD_CD_MAP` 250개 시/군/구 전체를 반복하며 지역별로 별도 `collect_source_items` 호출 + `collection_jobs` 로그를 남긴다(세종처럼 구/군 없는 지역도 처리). `docker-compose.yml`에 `worker`/`beat` 서비스 추가해 `docker compose up -d`로 함께 기동됨.
   - **실제 beat 발화를 로그로 확인**(9시간 경과 후): 온비드 매시간 트리거 8회 정상 발화. 국토부 전국 순회(daily crontab)는 beat 시작 시각(00:44 KST, 그날 3시가 이미 지난 뒤)이 기준점이 되어 다음날 03:00 KST로 예약됨 — 버그 아니고 정상 동작(`celerybeat-schedule` shelve 파일의 `last_run_at`으로 확인).
   - **버그 발견 및 수정**: 온비드 매시간 태스크 8회 중 정확히 절반(홀수 회차)이 `RuntimeError: ... attached to a different loop`로 실패하는 것을 발견. Celery worker가 매 태스크마다 `asyncio.run()`으로 새 이벤트 루프를 만드는데, `app/core/database.py`의 전역 `AsyncEngine`(커넥션 풀)은 프로세스 전역으로 재사용되어 이전(이미 닫힌) 루프에 묶인 커넥션을 다음 실행에서 재사용하려다 충돌하는 문제였다. `tasks.py`의 각 Celery task가 실행 끝에 `engine.dispose()`하도록 `_run_and_dispose_engine()` 헬퍼로 감싸서 해결 — FastAPI 앱(`app` 컨테이너)은 별도 프로세스라 이 dispose가 영향 주지 않음. 수정 후 동일 태스크 4회 연속 실행 전부 성공 확인, `/run-checks` 재확인 완료.
